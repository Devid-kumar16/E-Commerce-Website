  ...
  return (
    <div className="bg-white rounded shadow overflow-hidden flex flex-col">
      <Link
        ...
        className="block h-44 flex items-center justify-center bg-gray-100 overflow-hidden"
      >
        <SafeImage
...
function CategoryPage({ onAddToCart }) {
  ...
  const mapped =
    CATEGORY_SLUG_TO_NAMES[slug] ||
    [CATEGORY_SLUG_TO_NAME[slug] || slug];
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      <p className="mt-3 text-gray-700">
        Vendor profile and products (demo).
      </p>
    </div>
  );
}
...
/* ------------------ Conditional Header/Footer for auth + admin pages ------------------ */
function ConditionalHeader({ onSearch, cart }) {
  const location = useLocation();

  // Hide public store header on login, signup and admin pages
  if (
...
function ConditionalFooter() {
  ...
  // Hide footer on login, signup and admin pages
  if (
    location.pathname === "/login" ||
    location.pathname === "/signup" ||
    location.pathname.startsWith("/admin")
