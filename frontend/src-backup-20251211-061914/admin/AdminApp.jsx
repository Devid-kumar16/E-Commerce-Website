        ...
        <thead className="bg-gray-50">
          <tr>
            {columns.map((c) => (
              <th
                key={c.key}
                className="px-4 py-2 text-left font-semibold text-gray-500 text-xs uppercase tracking-wide"
              >
                {c.label}
              </th>
            ))}
              ...
              >
                {columns.map((c) => (
                  <td key={c.key} className="px-4 py-2">
                    {typeof c.render === "function" ? c.render(row, row) : row[c.key]}
                  </td>
                ))}
              </tr>
            ))
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        const ordList = orders.data || orders;
        const custList = customers.data || customers;

        if (!cancelled) {
          setStats({
            products: Array.isArray(prodList) ? prodList.length : prodList?.total ?? 0,
            categories: Array.isArray(catList) ? catList.length : catList?.total ?? 0,
            orders: Array.isArray(ordList) ? ordList.length : ordList?.total ?? 0,
            customers: Array.isArray(custList) ? custList.length : custList?.total ?? 0,
          });
        }
      } catch (err) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-3 items-center">
        <div className="flex items-center gap-2 px-3 py-2 bg-white rounded-full shadow-sm border text-sm">
          ...
          <input
            ...
            className="outline-none border-none bg-transparent text-sm w-48"
          />
        </div>
        <select
          value={statusFilter}
          onChange={(e) => setStatusFilter(e.target.value)}
          className="px-3 py-2 border rounded-full text-sm bg-white shadow-sm"
        >
              ...
              render: (p) => (
                <div className="flex flex-wrap gap-1 text-xs">
                  <button
                    type="button"
                    onClick={() => startEdit(p)}
                    className="px-2 py-1 rounded-full border border-gray-300 bg-white"
                  >
                    Edit
                  </button>
                  <button
                    ...
                    className="px-2 py-1 rounded-full border border-emerald-200 bg-emerald-50"
                  >
                    Publish
                  </button>
                  <button
                    ...
                    className="px-2 py-1 rounded-full border border-amber-200 bg-amber-50"
                  >
                    Draft
                  </button>
                  <button
                    ...
                    className="px-2 py-1 rounded-full border border-gray-200 bg-gray-50"
                  >
                    Inactive
                  </button>
                </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
        <h2 className="font-semibold mb-3 text-gray-800 text-sm">
          ...
          },
          {
            ...
            render: (c) => (
              <button
                type="button"
                onClick={() => startEdit(c)}
                className="text-xs px-2 py-1 rounded-full border border-gray-300 bg-white"
              >
                Edit
              </button>
            ),
...
export default function AdminApp() {
  ...
  if (!user) {
    // if user not loaded yet, redirect to login
    ...
    <div className="min-h-screen bg-slate-100 flex">
      ...
      <aside className="admin-sidebar">
        <div className="sidebar-brand">
          <div className="brand-logo">{user?.name ? user.name.slice(0, 2).toUpperCase() : "ES"}</div>
          <div>
            <div className="brand-title">E-Store Admin</div>
            <div className="brand-sub">Logged in as {user?.name} ({user?.role})</div>
          </div>
        </div>
      ...
      <main className="flex-1 admin-main">
        <header className="admin-main-header">
          <Link to="/" className="admin-back-btn">? Back to store</Link>
          <div className="admin-main-spacer" />
          <div className="admin-main-avatar">{user?.name ? user.name[0].toUpperCase() : "A"}</div>
        </header>
        ...
        <section className="admin-main-content">
          <Routes>
            ...
            {/* If you want the OrdersPage (separate page) use a different path */}
            <Route path="orders/page" element={<OrdersPage />} />
            <Route path="customers" element={<AdminCustomers />} />
            {/* fallback to admin root */}
            <Route path="*" element={<Navigate to="." replace />} />
            <Route path="/manage-users" element={<AdminManagement />} />
