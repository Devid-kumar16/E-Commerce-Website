...
/* auth localStorage helpers: stored user object */
const readAuth = () => {
  try {
  ...
  const login = async ({ email, password }) => {
    try {
      ...
      if (err instanceof TypeError) {
        throw new Error(
          "Backend se connect nahi ho paaya. Kya server (port 5000) chal raha hai?"
        );
      }
      throw err;
    }
    ...
    <header className="main-header">
      <div className="main-header-inner">
        ...
        <div className="relative">
          ...
          {openCats && (
            <div className="absolute left-0 mt-2 w-64 bg-white rounded shadow-lg border p-3 z-50 text-gray-900">
        ...
        <div className="main-header-right">
          <Link to="/wishlist" className="pill-btn">
            Wishlist
          </Link>
                ...
                {user ? (
                  ...
                  <>
                    ...
                    <div className="account-menu-sub">
                      Login or create an account to track orders.
                    </div>
                    <div className="account-menu-actions">
                      <Link
                        to="/login"
                        onClick={() => setOpenAccount(false)}
                        className="account-menu-btn primary"
                      >
                        Login
                      </Link>
                      <Link
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <aside className="bg-white rounded shadow p-4">
          ...
          <div className="mt-3 space-y-3">
            {similar.length === 0 ? (
              <div className="text-sm text-gray-500">No similar products</div>
            ) : (
              similar.map((s) => (
...
function Login() {
  ...
  const submit = async (e) => {
    ...
    setLoading(true);
    try {
      const logged = await login(form); // { id, name, email, role }
  ...
  return (
    <div className="account-page">
      <div className="account-card">
        ...
        <p className="account-subtitle">
          Shop faster, track orders and manage returns easily.
        </p>
  ...
  const placeOrder = () => {
    ...
    setTimeout(() => {
      ...
      if (idx !== -1) {
        ...
        if (window.showEstoreToast)
          window.showEstoreToast("Order placed! Order id: " + order.id);
